...
import BitcoinLockTransactionModel from '../models/CardanoLockTransactionModel';
...
import SavedLockType from '../models/SavedLockType';
...
export default class LockMonitor {
  ...
  private async periodicPoll (): Promise<void> {
    try {
      ...
      EventEmitter.emit(EventCode.CardanoLockMonitorLoopSuccess);
    } catch (e) {
      EventEmitter.emit(EventCode.CardanoLockMonitorLoopFailure);
  ...
  private async getCurrentLockState (): Promise<LockState> {
    ...
    try {
      ...
      if (e instanceof SidetreeError &&
        (e.code === ErrorCode.LockResolverTransactionNotConfirmed || e.code === ErrorCode.LockResolverTransactionNotFound)) {
  ...
  private async rebroadcastTransaction (lastSavedLock: SavedLockModel): Promise<void> {
    ...
    await this.cardanoClient.broadcastLockTransaction(lockTransactionFromLastSavedLock);
  }

  private async isTransactionBroadcasted (transactionId: string): Promise<boolean> {
    try {
      await this.cardanoClient.getRawTransaction(transactionId);
  ...
  private async handleCreatingNewLock (desiredLockAmountInSatoshis: number): Promise<SavedLockModel> {
    ...
    const walletBalance = await this.cardanoClient.getBalanceInLovelaces();
    ...
    const height = await this.cardanoClient.getLedgerTip();
    const lockTransaction = await this.cardanoClient.createLockTransaction(
                                                          totalLockAmount, 
                                                          this.versionManager.getLockDurationInBlocks(height)
                                  );
    ...
    EventEmitter.emit(EventCode.CardanoLockMonitorNewLock);
  ...
  private async renewLock (currentValueTimeLock: ValueTimeLockModel, desiredLockAmountInSatoshis: number): Promise<SavedLockModel> {
    ...
    const newLockDuration = this.versionManager.getLockDurationInBlocks(await this.cardanoClient.getLedgerTip());
    const relockTransaction =
      await this.cardanoClient.createRelockTransaction(
  ...
  private async releaseLock (currentValueTimeLock: ValueTimeLockModel, desiredLockAmountInSatoshis: number): Promise<SavedLockModel> {
    ...
    const releaseLockTransaction =
      await this.cardanoClient.createReleaseLockTransaction(
        currentLockIdentifier.transactionId,
        currentLockDuration);
    ...
    EventEmitter.emit(EventCode.CardanoLockMonitorLockReleased);
  ...
  private async saveThenBroadcastTransaction (
    ...
    await this.cardanoClient.broadcastLockTransaction(lockTransaction);

    return lockInfoToSave;
  }

  private async isUnlockTimeReached (unlockTransactionTime: number): Promise<boolean> {
    const currentTip = await this.cardanoClient.getLedgerTip();

    Logger.info(`Current block: ${currentTip}; Current lock's unlock block: ${unlockTransactionTime}`);

    return currentTip.time >= unlockTransactionTime;
  }
}
